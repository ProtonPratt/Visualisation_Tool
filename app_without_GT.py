...
filename = 'data/pre_computed_json_rohan_500_Hi_sam.json'
...
@app.route('/')
def index():
    return redirect(url_for('view_image', index=0, mode='predicted', metric='none'))
...
def view_image(index):
    ...
    view_mode = request.args.get('mode', 'predicted')
    
    print(index, metric, view_mode, order, image_name, image_language, selected_lang)
    # print(image_names)

    return render_template('image_viewer_hisam.html',
                           recall=recall,
                           precision=precision,
                           miou=miou,
...
def get_processed_image(index, mode):
    ...
    if mode == 'original':
        image_path = os.path.join('static', 'rohan_dataset_selected_500', image_name)
...
if __name__ == '__main__':
    app.run(debug=True, port=4070, host='0.0.0.0')
